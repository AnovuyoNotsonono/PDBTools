...
def get_pdb(ID='PDB_options'):
  ...
  #Checking whether a file exists before downloading using the "os" module, this line ONLY executes
  ...
  #Error handling: handling a FileNotFoundError that happens when no argument is given during a function call.
    try:
...
#Defining a second function that allows to user to acces the PDB details by using the ID and the option number        
def pdb_details(ID, key):
    ...
    # Calling and assigning get_pdb() function into a variable to use it on this second function
    output1=get_pdb(ID)
...
def protein_residues(ID,chain_id):
    ...
    #Iterating through the "standard_amino_acids" dictionary to acces those keys existing on the "three-letter_residues"
...
#Write a FASTA-formatted file, given a protein chain ID and an output file name.
#If no chain is given, it saves each chain as an entry in a single fasta file. Use the PDB header,
#and the relevant chain ID(s) as the definition line(s).
def pdb_chains(ID, output_filename, chain=None):
    ...
    chain_ids=""
    #Defining a pdb header
    pdb_HEADER=output1[0]
    #Isolating lines that satisfy the following conditions 
    for line in output1:
        if line.startswith("SEQRES") and chain==None:
            #Isolating chains from the pdb file and storing them in the empty string created
            chains +=line[19:70]
            #Isolating chain ids corresponding to the chains and storing them in aseparate empty string
            chain_ids+=line[11]   
    #Creating and saving the new fasta file that strores the pdb_header and chain_ids as definition lines and 
     #the isolated chains
...
#Print or write the relevant lines to a file, given a chain ID only, or a chain ID and a HETATM/ATOM record type.
#The choice of reading or writing is to be specified by the user. 
...
def change_chain_id(ID,record_type,chain_id, new_chain_id):
    """change_chain_id takes as positional inputs PDB ID,record_type(ATOM/HETATM), chain_id to be changed
       and a new chain_id, this function returns a file written to it the PDB contents with the altered 
       chain id"""
    ...
    for line in output1:
        #Creating an empty string to store lines with the altered chains
        altered_chain_id=""
